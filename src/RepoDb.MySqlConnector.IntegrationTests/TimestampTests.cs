...
internal class TimestampTestTable
{
    public int Id { get; set; }
    public string Name { get; set; }
    public DateTime CreatedAt { get; set; }
    public DateTime UpdatedAt { get; set; }
    public DateTime? OnUpdateOnly { get; set; }
}
...
public class TimestampTests
{
    ...
    private void CreateTestTable()
    {
        ...
        connection.DeleteAll<TimestampTestTable>();
    }
    ...
    public void TestDefaultCurrentTimestampCanBeOverridden()
    {
        ...
        connection.Insert(new TimestampTestTable(), trace: new DiagnosticsTracer());

        // Verify the explicit timestamp was used
        querySql = $"SELECT CreatedAt FROM `{TestTableName}` WHERE Id = @Id";
        createdAt = connection.ExecuteScalar<DateTime>(querySql, new { Id = id+1 });

        // The CreatedAt should be filled in with 'now'
        Assert.IsLessThanOrEqualTo(DateTime.Now + TimeSpan.FromSeconds(1), createdAt, "DEFAULT CURRENT_TIMESTAMP should allow explicit values to be provided during INSERT");
        Assert.IsGreaterThan(DateTime.Now - TimeSpan.FromMinutes(1), createdAt, "DEFAULT CURRENT_TIMESTAMP should allow explicit values to be provided during INSERT");
    }
