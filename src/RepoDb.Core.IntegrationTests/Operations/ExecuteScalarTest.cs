...
public class ExecuteScalarTest
{
    ...
    public void ThrowExceptionOnTestSqlConnectionExecuteScalarIfTheParametersAreNotDefined()
    {
        using var connection = new SqlConnection(Database.ConnectionStringForRepoDb);
        // Act
        Assert.ThrowsExactly<SqlException>(() => connection.ExecuteScalar("SELECT * FROM [sc].[IdentityTable] WHERE (Id = @Id);"));
    }
    ...
    public void ThrowExceptionOnTestSqlConnectionExecuteScalarTIfTheParametersAreNotDefined()
    {
        using var connection = new SqlConnection(Database.ConnectionStringForRepoDb);
        // Act
        Assert.ThrowsExactly<SqlException>(() => connection.ExecuteScalar<object>("SELECT * FROM [sc].[IdentityTable] WHERE (Id = @Id);"));
    }
