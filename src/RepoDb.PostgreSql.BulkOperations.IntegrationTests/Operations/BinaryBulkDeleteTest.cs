...
[TestClass]
public class BinaryBulkDeleteTest : TestBase
{
    ...
    public async Task TestBinaryBulkDeleteAsyncTableNameWithSchema()
    {
        ...
        // Act
        var result = await connection.BinaryBulkDeleteAsync(tableName, entities, cancellationToken: TestContext.CancellationToken);

        // Assert
        Assert.AreEqual(entities.Count, result);

        // Assert
        var countResult = await connection.CountAllAsync(tableName, cancellationToken: TestContext.CancellationToken);
        Assert.AreEqual(0, countResult);
    }
    ...
    public async Task TestBinaryBulkDeleteAsyncWithKeepIdentityFalse()
    {
        ...
        result = await connection.BinaryBulkDeleteAsync(
            tableName,
            entities: entities,
            keepIdentity: false, cancellationToken: TestContext.CancellationToken);
