...
public class SqlServerDbInstance : DbInstance<SqlConnection>
{
    ...
    public SqlServerDbInstance()
    {
        ...
        // Owner connection (repodb_defs_owner with db_owner privileges)
        ConnectionString =
    ...
    protected override async Task CreateUserDatabase(DbConnection sql)
    {
        ...
        IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = 'repodb_defs_owner')
        BEGIN
            CREATE LOGIN [repodb_defs_owner] WITH PASSWORD = '8A5F2E1B-42C3-4D9E-B156-7C3F9D0A2E4B';
        END");

        // Create limited user login if it doesn't exist
        await sql.ExecuteNonQueryAsync(@"
        IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = 'repodb_defs_user')
        BEGIN
            CREATE LOGIN [repodb_defs_user] WITH PASSWORD = '0608B012-05D2-4023-A451';
        END");
        ...
        await sql.ExecuteNonQueryAsync($@"
        IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = 'repodb_defs_owner')
        BEGIN
            CREATE USER [repodb_defs_owner] FOR LOGIN [repodb_defs_owner];
            -- Add user to db_owner role for setup operations
            ALTER ROLE [db_owner] ADD MEMBER [repodb_defs_owner];
        END");

        // Create limited user in the test database with minimal permissions
        await sql.ExecuteNonQueryAsync($@"
        IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = 'repodb_defs_user')
        BEGIN
            CREATE USER [repodb_defs_user] FOR LOGIN [repodb_defs_user];
            -- Grant permission to connect to database
            GRANT CONNECT TO [repodb_defs_user];
            -- Add user to db_datawriter and db_datareader roles for basic data operations only
            ALTER ROLE [db_datawriter] ADD MEMBER [repodb_defs_user];
            ALTER ROLE [db_datareader] ADD MEMBER [repodb_defs_user];
        END");
