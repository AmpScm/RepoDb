...
public class PostgreSqlDbInstance : DbInstance<NpgsqlConnection>
{
    ...
    protected override async Task CreateUserDatabase(DbConnection sql)
    {
        ...
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_user WHERE usename = 'repodb_pg_owner') THEN
                CREATE USER ""repodb_pg_owner"" WITH PASSWORD '8A5F2E1B-42C3-4D9E-B156-7C3F9D0A2E4B' LOGIN;
            END IF;
        END
        $$;");
        ...
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_user WHERE usename = 'repodb_pg_user') THEN
                CREATE USER ""repodb_pg_user"" WITH PASSWORD '0608B012-05D2-4023-A451' LOGIN;
            END IF;
        END
        $$;");
        ...
        await sql.ExecuteNonQueryAsync($@"
        GRANT CONNECT ON DATABASE ""{DatabaseName}"" TO ""repodb_pg_owner"";
        GRANT CONNECT ON DATABASE ""{DatabaseName}"" TO ""repodb_pg_user"";");
        ...
        await sql.ExecuteNonQueryAsync($@"
        GRANT ALL PRIVILEGES ON SCHEMA public TO ""repodb_pg_owner"";
        ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ""repodb_pg_owner"";
        ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ""repodb_pg_owner"";
        ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT EXECUTE ON FUNCTIONS TO ""repodb_pg_owner"";");

        // Grant limited user permissions in the context of the new database
        await sql.ExecuteNonQueryAsync($@"
        GRANT USAGE ON SCHEMA public TO ""repodb_pg_user"";
        ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ""repodb_pg_user"";
        ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE ON SEQUENCES TO ""repodb_pg_user"";
        GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ""repodb_pg_user"";
        GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO ""repodb_pg_user"";"
        );
