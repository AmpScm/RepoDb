...
public class ExecuteReaderTest
{
    ...
    public async Task TestSqlServerConnectionExecuteReaderAsync()
    {
        ...
        using var connection = new SqlConnection(Database.ConnectionString);
        // Act
        using var reader = await connection.ExecuteReaderAsync("SELECT \"Id\", \"ColumnInt\", \"ColumnDate\" FROM \"CompleteTable\";", cancellationToken: TestContext.CancellationToken);
        while (reader.Read())
        {
            // Act
            var id = reader.GetInt32(0);
    ...
    public async Task TestSqlServerConnectionExecuteReaderAsyncWithMultipleStatements()
    {
        ...
        using var connection = new SqlConnection(Database.ConnectionString);
        // Act
        using var reader = await connection.ExecuteReaderAsync("SELECT \"Id\", \"ColumnInt\", \"ColumnDate\" FROM \"CompleteTable\"; SELECT \"Id\", \"ColumnInt\", \"ColumnDate\" FROM \"CompleteTable\";", cancellationToken: TestContext.CancellationToken);
        do
        {
            while (reader.Read())
            {
